...
format_diff() {
	...
	if [[ $CLANG_FORMAT_MAJOR_VERSION -gt "9" ]]; then
		local_format="$(docker run -i --volumes-from ${CID} -w "$(pwd)" --rm ghcr.io/jidicula/clang-format:"$CLANG_FORMAT_MAJOR_VERSION" -n --Werror --style=file --fallback-style="$FALLBACK_STYLE" "${filepath}")"
	else # Versions below 9 don't have dry run
		formatted="$(docker run -i --volumes-from ${CID} -w "$(pwd)" --rm ghcr.io/jidicula/clang-format:"$CLANG_FORMAT_MAJOR_VERSION" --style=file --fallback-style="$FALLBACK_STYLE" "${filepath}")"
